        ...
        public static List<Tuple<string, double>> CalculatePFI(MLContext mlContext, IDataView trainData, ITransformer model, string labelColumnName)
        {
         ...
         .PermutationFeatureImportanceNonCalibrated(
            ...
            var featureImportanceMetrics =
                 ...
                 .Select((kvp) => new { kvp.Key, kvp.Value.AreaUnderRocCurve })
                 .OrderByDescending(myFeatures => Math.Abs(myFeatures.AreaUnderRocCurve.Mean));

            var featurePFI = new List<Tuple<string, double>>();
            foreach (var feature in featureImportanceMetrics)
            {
                var pfiValue = Math.Abs(feature.AreaUnderRocCurve.Mean);
